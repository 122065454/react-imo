...
const OverList = ({ param })=> {
  ...
  const cardRender = () => {
    ...
    arrayList.forEach((item, index) => {
      list.push(
        <div className={styles.mb30} style={terminal && (index+1)%3 !== 0 ? contSet.mrDis(30) : {}} key={`cardRender${Math.random()}`}><Card param={item} subscript={3} /></div>)
    })
    return list
  }
  ...
  return (
    <div className={`${terminal ? styles.range : ''}`}>
      {size > 0 ? cardRender() : (<div className={styles.noData}><Nodata /></div>)}
    </div>
  )
}

export default observer(OverList)
